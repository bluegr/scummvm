 ...
 *
 * $URL: https://scummvm-dgds.googlecode.com/svn/trunk/resourcemanager.cpp $
 * $Id: resourcemanager.cpp 12 2010-01-23 12:01:58Z alexbevi $
 *
 */
...
ResourceManager::ResourceManager() {
	...
	if (Common::File::exists("volume.rmf"))
		indexFile.open("volume.rmf");
	else if (Common::File::exists("volume.vga"))
		indexFile.open("volume.vga");
	else if (Common::File::exists("volume.map"))
		indexFile.open("volume.map");
	else
		error("[ResourceManager] No Valid Index Files Found");
...
Resource *ResourceManager::load(Common::String const &resourceName) {
	Resource *res = NULL;
	...
	if (Common::File::exists(resourceName)) {
		...
		res = new Resource(resourceName, file, tmp[3] == ':');
...
void ResourceManager::dumpResources(Common::String const &path, bool subres) {
	...
	while (iter != _resourceFiles.end()) {
		Resource *res = getResource(iter->_key, iter->_value);
